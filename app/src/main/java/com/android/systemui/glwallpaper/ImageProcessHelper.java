...
public class ImageProcessHelper {
    ...
    public static class Threshold {
        public static ArrayList<Integer> getHistogramPublic(Bitmap bitmap) {
            Bitmap grayscale = toGrayscale(bitmap);
            return getHistogramBySafe(grayscale);
        }
        public static boolean isSafe(Bitmap bitmap) {
            Bitmap grayscale = toGrayscale(bitmap);
            return checkHistogramSafe(grayscale);
        }
        ...
        private static boolean checkHistogramSafe(Bitmap grayscale) {
        ...
        private static ArrayList<Integer> getHistogramBySafe(Bitmap grayscale) {
            int width = grayscale.getWidth();
            int height = grayscale.getHeight();
            ...
            for (int row = 0; row < height; row++) {
                for (int col = 0; col < width; col++) {
                    ...
                    int y = Color.red(pixel) + Color.green(pixel) + Color.blue(pixel);
                    if (histogram.size() <= y) {
                        for (int i = histogram.size(); i <= y; i += 1) {
                            histogram.add(0);
                        }
                    }
                    if (y >= 256) {
                        Log.d("SafeMountain", "Exception Detected (r:" + row + ",c:" + col + ")");
                    }
                    histogram.set(y, histogram.get(y) + 1);
                }
            }
        ...
        private static int[] getHistogram(Bitmap grayscale) {
            int width = grayscale.getWidth();
            int height = grayscale.getHeight();
        ...
        private static boolean isSolidColor(Bitmap bitmap, int[] histogram) {
